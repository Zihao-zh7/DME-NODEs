...
def generate_dataset(data, args):
    ...
    # 检查是否使用固定连续七天数据
    if hasattr(args, 'use_fixed_week') and args.use_fixed_week:
        # 使用固定连续七天的数据
        # 计算每天的时间步数（假设数据是每小时一个点）
        steps_per_day = getattr(args, 'days_per_hour', 24)
        total_days = 7
        
        # 计算起始和结束时间步
        start_day = getattr(args, 'start_day', 0)
        start_step = start_day * steps_per_day
        end_step = start_step + total_days * steps_per_day
        
        # 确保不超出数据范围
        if end_step > data.shape[0]:
            print(f"Warning: Requested 7-day period exceeds data length. Using available data from day {start_day}")
            end_step = data.shape[0]
        
        # 使用固定时间段的数据
        fixed_data = data[start_step:end_step]
        
        # 计算训练、验证、测试的划分
        total_steps = fixed_data.shape[0]
        train_end = int(total_steps * 0.6)
        valid_end = int(total_steps * 0.8)
        
        train_dataset = MyDataset(fixed_data, 0, train_end, his_length, pred_length)
        train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

        valid_dataset = MyDataset(fixed_data, train_end, valid_end, his_length, pred_length)
        valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=True)

        test_dataset = MyDataset(fixed_data, valid_end, total_steps, his_length, pred_length)
        test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True)
        
        print(f"Using fixed 7-day period: Day {start_day} to Day {start_day + total_days}")
        print(f"Data range: {start_step} to {end_step} time steps")
